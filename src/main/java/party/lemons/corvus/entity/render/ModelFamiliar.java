...
public class ModelFamiliar extends ModelBase
{
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn)
	{
		...
		this.wolfTail.rotateAngleX = (0.6F + MathHelper.cos(limbSwing * 0.6662F) * 0.3F * limbSwingAmount) + ((-0.5F + (float)Math.sin(ageInTicks/ 4)) / 25F);
		this.wolfTail.rotateAngleZ =  ((-0.5F + (float)Math.sin(ageInTicks/ 8)) / 25F);
	}
}